...
def one(path):
    ...
    mod=load_source('replay_v38',ROOT/'src/main3.py');s=mod.GameSession(origin=1)
    ...
    print('[one] '+json.dumps(report,ensure_ascii=False),flush=True);return report
if __name__=='__main__':
    paths=sorted(ROOT.glob('log/round14/game*/ally*'))
    if not paths:raise SystemExit('[replay_round14] no Round14 print logs provided')
    ...
    out=ROOT/'runs/v38';out.mkdir(parents=True,exist_ok=True);(out/'replay_validation.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
    ok=not any(r['failures'] or r['task_mismatches'] or r['full_health_medicine'] for r in rows)
    print('[replay_round14] complete passed='+str(ok),flush=True);raise SystemExit(0 if ok else 1)
