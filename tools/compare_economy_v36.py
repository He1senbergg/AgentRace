...
Identical night release times from V3.5 Round12 observations, no damage, no robot AI.
...
W=Path(__file__).resolve().parents[1];OUT=W/'runs/v36_economy';OUT.mkdir(parents=True,exist_ok=True);G=json.loads((W/'tests_v36/fixtures/economy_inputs.json').read_text())
def load(name,path):
...
def run_one(name,version,turns=590):
 g=G[name];mod=load('econ_'+version, W/('tests_v36/fixtures/baseline_v35.py' if version=='old' else 'src/main3.py'))
 ...
 for n in range(1,turns+1):
  ...
  if n in [70,130,200,260,330,390,460,520,590]:
   roles=engine.data['teamOur']['roles'];stats=dict(round=n,gold=engine.data['teamOur']['goldNum'],tasks=len(ended),base_level=next(r['level'] for r in roles if r['roleType']=='station'),weapons=sorted((r['level'] for r in roles if r['roleType']=='rocket'),reverse=True),walls=sum(r['roleType']=='wall' for r in roles),front_levels=[(r['pos'],r['level'],r['health']) for r in roles if r['roleType']=='wall'],mining_gold=engine.stats['mineral_revenue'],maintenance=sum(engine.stats['buy:'+k]*dict((x['name'],x['price']) for x in initial['weaponShopList'])[k] for k in ['WallFixer','WallUpgradeVoucher1','WallUpgradeVoucher2']))
 ...
 result=dict(game=name,version=version,source_sha256=hashlib.sha256((W/('tests_v36/fixtures/baseline_v35.py' if version=='old' else 'src/main3.py')).read_bytes()).hexdigest(),elapsed=time.monotonic()-t0,history=history,tasks_end=ended,night_work_commands=dict(work),milestones=milestones,rejected=engine.rejected,stats=dict(engine.stats))
...
def pair(name):
 baseline=OUT/('econ_'+name+'_old.json')
 old=json.loads(baseline.read_text()) if '--reuse-baseline' in sys.argv and baseline.exists() else run_one(name,'old')
 return [old,run_one(name,'new')]
if __name__=='__main__':
 names=[x for x in sys.argv[1:] if x!='--reuse-baseline'] or sorted(G)
 with concurrent.futures.ProcessPoolExecutor(max_workers=4) as ex:results=[r for pair_results in ex.map(pair,names) for r in pair_results]
