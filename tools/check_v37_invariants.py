...
oldpath=ROOT/'tests/tests_v37/fixtures/baseline_v36.py'
...
def run_invariants():
    ...
    rng=random.Random(37013);counts=0;max_ms=0;failures=[]
    for i in range(400):
    ...
    result=dict(runtime=sys.version,source_sha256=hashlib.sha256((ROOT/'src/main3.py').read_bytes()).hexdigest(),
                unchanged_kernels=list(before),mutation_cases=400,passed=counts,failures=failures,max_ms=max_ms,
                scope='Fixed-seed mutated actual snapshots, clean memory per case. Cash/legality/idempotency/health guards, not win-rate.')
    (ROOT/'runs/v37/invariants.json').write_text(json.dumps(result,ensure_ascii=False,indent=2))
