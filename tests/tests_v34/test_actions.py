...
def role(actor, kind, x, y, **fields):
    return dict(id=actor, roleType=kind, pos=dict(x=x, y=y), health=100, backpack=[], **fields)

def state(*roles, **fields):
    return dict(roundNo=0, teamOur=dict(teamId=7, type='challenger', goldNum=75, roles=list(roles)), mapInfo=dict(zones=[]), **fields)
...
class ActionTests(unittest.TestCase):
        ...
        for kind, counts in (('gatling', (1, 2, 3)), ('railgun', (1, 1, 1)), ('rocket', (1, 2, 3))):
            for level, required in enumerate(counts, 1):
                ...
                for count in (0, 1, 2, 3, 4):
                    ...
                    self.assertEqual(validator(data, main.Phase(1, 71)).add('2', command), count == required, (kind, level, count))
    ...
    def test_health_numeric_boundaries_do_not_raise(self):
        for value, allowed in ((10 ** 400, True), (float('inf'), False), (float('nan'), False), (True, False), (0, False)):
            with self.subTest(value=str(value)):
                ...
                self.assertEqual(validator(data, main.Phase(1, 71)).add('2', target('attack', 2, 2, controllerId='1')), allowed)
    ...
    def test_shared_gold_and_rejection_is_atomic(self):
        data = state(role(1, 'worker', 1, 1), role(2, 'pioneer', 2, 1), weaponShopList=[dict(name='Medicine', price=40)])
    ...
    def test_capacity_multiset_and_no_sale_credit(self):
        data = state(role(1, 'worker', 1, 1, backPackCapability=2), role(2, 'worker', 2, 1), vendorShopList=[dict(name='iron', price=100)], weaponShopList=[dict(name='Medicine', price=100)])
    ...
    def test_attack_cone_controller_cooldown_and_runtime_range(self):
        data = state(role(1, 'worker', 9, 10), role(2, 'gatling', 10, 10, level=2, attackRange=4), role(3, 'rocket', 8, 10, level='level1', cooldown=1))
        ...
        self.assertTrue(v.add('2', command))
    ...
    def test_build_regions_counts_budget_and_unknown_rules(self):
        data = state(role(1, 'worker', 9, 11), role(2, 'worker', 12, 11), role(3, 'station', 10, 10, level=1))
