# Selected core regression; no removed legacy/Flask branches.
...
class MemoryTests(unittest.TestCase):
    ...
    def test_callback_integration_and_duplicate_isolation(self):
        ...
        session = main.GameSession(plan, strategy_mode='survival')
    ...
    def test_parallel_duplicates_run_planner_once(self):
        calls = []
        session = main.GameSession(lambda w, m: (calls.append(m.last_round), main.empty_response())[1], strategy_mode='survival')
    ...
    def test_transaction_rolls_back_planner_and_validation_failures(self):
        session = main.GameSession(strategy_mode='survival')
    ...
    def test_history_is_not_current_obstacle_and_news_deduplicates(self):
        session = main.GameSession(origin=1, strategy_mode='survival')
        observed = []
        session.planner = lambda w, m: (observed.append(set(w.occupied)), main.empty_response())[1]
        news = {'officialNews': '铁价变化', 'folkLegends': '相同线索'}
        session.handle(request_at(1, worldNews=news, teamEnemy={'roles': [{'id': 9, 'roleType': 'worker', 'health': 100, 'pos': {'x': 8, 'y': 9}}]}))
    ...
    def test_feedback_does_not_infer_success_or_associate_across_gap(self):
        response = {'roleCommandMap': {'10010': {'action': 'move', 'targetPos': [{'x': 1, 'y': 1}]}}, 'prompt': '', 'executeCmd': ''}
        session = main.GameSession(lambda w, m: deepcopy(response), strategy_mode='survival')
        observed = {'teamId': 7, 'type': 'challenger', 'roles': [{'id': 10010, 'roleType': 'worker', 'pos': {'x': 0, 'y': 0}, 'health': 220}]}
    ...
    def test_identity_change_and_round_regression_reset_history(self):
        for next_request in (request_at(0), request_at(9, teamOur={'teamId': 8, 'type': 'challenger'}), request_at(9, teamOur={'teamId': 7, 'type': 'defender'})):
            session = main.GameSession(strategy_mode='survival')
    ...
    def test_missing_invalid_input_and_origin(self):
        session = main.GameSession(strategy_mode='survival')
        for data in (None, [], {}, request_at(True), request_at(-1), request_at(1301), request_at(1, teamOur={'teamId': False, 'type': 'challenger'}), request_at(1, teamOur={'teamId': 1, 'type': []})):
        ...
        confirmed = main.GameSession(origin=1, strategy_mode='survival')
        ...
        inferred = main.GameSession(strategy_mode='survival')
    ...
    def test_unserializable_response_does_not_commit(self):
        session = main.GameSession(lambda w, m: {'roleCommandMap': {}, 'prompt': '\ud800', 'executeCmd': ''}, strategy_mode='survival')
