...
.venv\Scripts\python.exe src\main.py 8080
```
...
保留`run.sh`和`src/main.py`的相对目录，另备requirements-dev.txt供环境复现。主程序只依赖标准库与Flask及其传递依赖，不依赖AI Spec、Official、docs或tests。
