...
保留单文件平台入口和原协议、任务、机器人战斗计算；仅增强SurvivalPlanner。后排三炮使用两个间隔，普通C14、富裕G18双通道，拒绝已在闭环回防中失败的G19。墙体最低配置是有限预算/背包/配送路径工程；基地券是个人持有的动态阈值保险，短程阻塞采用逐帧让路。PvP是清场后至多单炮的可关闭探针，不把敌方血量变化当作因果证明。偏矿只降级不假装修图。

以下为历史设计，冲突时以V3.8报告和当前实现为准。

# V3.1 调度修复（2026-09-14，当前）

当前生产版本为 v3.1-reviewed-fix；详细取舍与残留风险见 V3_1_FIX_REPORT.md。

- stone 预留从 job.kind 中分离，按观察库存/剩余缺口/批次上限取最小值。清包出售铜铁时不取消建墙；已携带石料的批次暂缓不转普通采矿；空包时允许保留任务的临时筹资。
- 已购券的成本预算为实际动态去程 + 一次 use + 到指定防御位的回程，不加采购余量，不取消日落截止。动作成功才占用目标；旧目标不通/超时后继续考虑其他目标。
- 夜间优先级：相邻且持券的基地救急、角色自救/规避、可用火力、支援、空闲角色普通维护、回位。救急 65% 阈值沿用原采购规则，是策略启发式，不是官方血量条件。
- 交付和开火均避开本回合已修改建筑；拒绝事件不被后续位置状态覆盖。旧协议/任务/布局/伤害计算保持不变。

以下设计记录属于旧版本，当前状态以 STATUS.md 为准。

---

# V3 当前架构补充（2026-09-13）

在原协议/观察/事务层之上新增 SurvivalPlanner，不直接重写旧 defense。真实入口通过环境变量选择默认 survival，旧库默认保持 defense。新策略使用观察驱动的持续任务、固定批量石料、资金到位后派工、全活人券库存扣减、三级资本缺口、角色/炮/位置联合分配与空闲先锋腾位。廉价非紧急墙券后置以防大额升级饥饿。Task R2 inspect 仅在 survival 使用一次只读检查包。

取舍、约束、已知歧义和验证边界见 V3_AUDIT_AND_FIX_REPORT.md。以下原设计作为旧模式与公共层背景保留。

---

# AgentRace 技术设计

## V2.4 离线可靠性修复设计（2026-09-12）

当前授权允许修复可复现的正确性问题；不以历史策略假设或本地模型充当实战效果证据。

- 建设任务是持久目标；缺钱、缺料、容量不足时允许工人在同一目标下临时筹资，不允许建造拒绝或 PAUSED 永久吞掉收入动作。筹资继续走现有经济规划器、动作权限和回防预算。
- 服务类资本预留须有通过分配可达性/期限检查、仍持匹配 job 的购买 owner；无角色、无商品、无法释放背包容量、改为回防或已持券的目标不冻结重复采购款。已使用券而等待观测验证的目标也不再次预留购买款。有效购买 owner 的优先级与资金缺口预留保留，所有花费只扣当前观测资金。建设预留仍以 BUILD_WEAPON job 存在为条件，未证明所有建设布局和筹资路线都能交付。
- 两火箭一电磁的补建按存活种类和同回合已接受建造计算缺口，使用同一类型选择 helper，避免按剩余总数错误补炮。
- 任务结束属于观测生命周期，不依赖调度器本回合是否调用 TaskPlanner。明确无活跃任务时归档未验证经验并清除旧 task；不得把提交合法性或金币变化标成解题成功。
- 日志异常不得影响任何 HTTP 阶段：before/after hook 及错误日志均隔离写入异常，保留合法 JSON 响应。
- 加特林/电磁伤害预测按中心连线穿过的格子内部判断拦截/能量消耗。整数几何避免浮点误差，边界/角点仅接触的判定与同回合死亡结算继续列为平台未决项。
- 补 default defense 的完整双半场观测集成；保留 V2.2 冻结 fixture，只对明确修复的方法定点调整 AST 门禁，保留其余定义与请求对照。

## V2.4 Sonnet E2–E5 落地（2026-09-12）

本次用户授权应用 sonnet5 补丁，取代此前审计文档仅实施 E1 的停止边界。默认仍 defense；shadow 的实验候选也采用新策略，实际返回仍由 legacy 决定。

- E2：保留有效旧施工目标，然后按 CanonicalLayout 序号选择可达前侧墙位；继续避让静态障碍、其他工人的预留和炮手站位，并检查出口。序号唯一，路程不是同一序号内实际有效的择优维度。
- E3：按当回合 ActionValidator 已接受的武器数量选择 rocket/rocket/railgun，维持名称映射与缺少 railgun 配置时的 rocket 回退。开局诊断同步采用两火箭一电磁炮。按数量选型不是受损重建后的组成优化。
- E4：仅 growth_mode 启用到基地锚点的距离权重 max(1,6-distance)，预测单次伤害足以击杀时乘2；保持原伤害模型、剩余HP试算、冷却/控制者/动作校验。该评分为实验启发式，不保证击杀数最优或每回合必有合法射击。
- E5：先锋未持目标物品且现有背包卖价不足当前资金缺口时，不参与该资本目标 owner 竞争；已持物品、现金足额、背包可补齐缺口者仍经过路线和截止校验。工人保持筹资能力，活动任务先锋继续隔离。
- 保留补丁提供的 helper 扩展：可注入容量目标、growth_mode 下仅预留买得起的基地券、墙采石上限20。生产调度不调用这些旧 helper 的建设/维修/预留方法，因此它们不解释 Round6 失败，也不改变默认 legacy 行为。

原 tests/fixtures/v22_baseline.py 及 SHA-256 保持不变；显式列出获准修改的方法，其他成员仍做 AST 冻结校验，新行为增加专项测试，legacy/shadow 实际响应继续和原基线比较。sonnet5 原始资料保留，历史测试声称不作为本轮验证结果。
...
## 2026-09-12：Task R1 任务链路增量设计

范围仅为任务。`TaskPlanner` 继续唯一持有开拓者任务状态及异步 pending；新 `task_protocol` 为纯辅助层（依赖 actions 的严格 JSON，不反向依赖 session/strategy），新 `task_trace` 只做显式启用的本地诊断。`GameSession.handle`、经济/防御/战斗/寻路和比赛对外协议不变。

模型控制协议保持一个 answer/command 主动作，新增可选 stdout 契约或候选来源信息。这些键只在 llmResp 字符串内部使用，不添加比赛 API 参数。自动提交只依赖已关联的完整 exitCode=0 工具来源，不依赖 skill 散文。完整 stdout 的来源等价不等于题意或评分正确。模型直接给出的答案保持原通道，由动作门禁检查合法性，不能通用检查题目真值。

任务内新增 sources/candidate/evidence_version/submissions/command_attempts；任务结束/换题不携带候选，回合缺失不消费旧返回。普通失败保留一次重试，126/127 无新成功观察不原样重发；成功的空 stdout 修复可以解除阻断但不作为答案。错误码 2 不区分错误/部分正确，动作合法不等于答案正确。

三个静态模板分别覆盖探索、格式修复、最终答案；文档、当前观察和历史分离。完整题目原文保留，辅助上下文 36,000 序列化字符预算、所有裁切显式标记；不声称等同 token 预算。跨任务只注入相关且标为未验证的方法，不注入保存的上题命令/观察。未实现通用语义压缩、schema 推断或经判题验证的 SOP。

审计开关 `AGENTRACE_TASK_TRACE_DIR` 默认空。开启后私有 JSONL 原文有单条/单文件大小上限及轮转，写入失败隔离于事务响应；不自动脱敏，不应写公开仓库/网络盘。完整说明和权衡见 [TASK_R1_CHANGELOG.md](TASK_R1_CHANGELOG.md)。

## Task R1 本地复核修正（2026-09-12）

- 最终阶段仍严格拒绝命令。若回复通过完整探索协议校验、仅因阶段禁止命令而失败，可独立验证其中显式候选的完整成功来源，并用于既有截止保底；其他格式错误不提取候选，不执行被拒命令。
- 候选 JSON 来源比较必须保留十进制数精度，并区分数字、布尔和字符串；不能通过 Python float 舍入把不同工具结果视为等价。精确原文仍允许非 JSON 任务答案。
- 已先运行能复现两项缺陷的回归，再修实现，并完成真实 HTTP 任务流程、两平台完整测试及单独的实现/规格复核；结果见 `TASK_R1_LOCAL_REVIEW.md`。

## 比赛入口仅保留端口（2026-09-12）

用户确认实机只传入端口，删除 main3.py 的策略、回合起点、墙成本、武器名称四个命令行覆盖项。依据 AI Spec §42–43，保留位置端口的合法范围校验、0.0.0.0 监听及无 debug/reloader 的启动方式。入口直接构造 GameSession()，沿用已确认的默认 defense、自动起点及 Rules()，不新增调试入口或替代环境变量。内部 GameSession/Rules 构造配置仍用于单元测试和历史策略对照；tools/replay_day.py 的离线回放选项属于独立工具，不受比赛入口清理影响。

验证包含端口边界、删除选项明确拒绝、默认规则与会话、真实默认 HTTP/直接脚本/run.sh，以及完整回归。冻结测试仅豁免明确改变的 main 函数体，保留接口签名与其他冻结检查。
