...
按用户指定的 Official/SDK_Python3_main3.py，入口统一为 `python main3.py <port>`（仓库内为 `python src/main3.py <port>`），`app.run(port=args.port)` 使用 Flask 默认 127.0.0.1。AI Spec §43.2 建议显式 0.0.0.0，但本次用户明确要求与官方骨架一致，因此采用官方默认；跨主机访问未验证。保留可选 CLI 配置、端口校验和 CLI 内 UTF-8 行缓冲设置；导入模块不会启动服务或重开共享输出流。run.sh 转发到 main3.py，测试导入同一文件。
